...
private:
  ...
  cv::Mat state_matrix_;  ///< x: The State estimates (상태 추정치)
  cv::Mat transition_matrix_;  ///< A: The State Transistion Matrix (상태 변환 행렬)
  cv::Mat measurement_matrix_;  ///< H: The Measurement Matrix (측정 행렬)
  cv::Mat process_noise_matrix_;  ///< Q: The Process Noise Covariance Matrix (과정 노이즈 공분산)
  cv::Mat measurement_noise_matrix_;  ///< R: The Measurement Noise Covariance Matrix (측정 노이즈 공분산)
  cv::Mat covariance_matrix_;  ///< P: The Error Covariance matrix (오차 공분산)
  cv::Mat kalman_gain_;  ///< K: The Kalman gain (칼만 이득)
};
} // XyCar
