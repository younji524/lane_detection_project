...
namespace XyCar
{
    ...
    void LaneManager::set_parameters(const YAML::Node& config)
    {
        ...
        k_frame_height = config["IMAGE"]["HEIGHT"].as<uint32_t>(); //for draw
        k_offset = config["LANE"]["OFFSET"].as<uint32_t>(); //for draw
    }
    ...
    void LaneManager::run()
    {
        ...
        while (ros::ok())
        {
            ...
            draw_line_slope(draw_image, lane_state.left_slope_, lane_state.left_intercept_, cv::Scalar(255,0,0), k_frame_height);
            draw_line_slope(draw_image, lane_state.right_slope_, lane_state.right_intercept_, cv::Scalar(255,0,0), k_frame_height);
            ...
            int32_t frame_centor = (k_frame_width / 2);
            ...
            draw_rectangle(draw_image, lane_centor, cv::Scalar(255,0,0),k_offset);
            draw_rectangle(draw_image, frame_centor, cv::Scalar(0,0,255),k_offset);
