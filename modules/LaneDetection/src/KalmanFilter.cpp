...
KalmanFilter::KalmanFilter(const YAML::Node &config) {
  ...
  transition_matrix_ = (cv::Mat_<PREC>(4, 4) << 1, dt_, 0, 0, 0, 1, 0, 0, 0, 0,
                        1, dt_, 0, 0, 0, 1);
...
void KalmanFilter::predict(PREC estimation_slope, PREC estimation_intercept) {
  state_matrix_ = transition_matrix_ *
                  (cv::Mat_<PREC>(4, 1) << estimation_slope, slope_derivative_,
                   estimation_intercept, intercept_derivative_);
  static const auto transition_matrix_t = transition_matrix_.t();
  covariance_matrix_ =
      transition_matrix_ * covariance_matrix_ * transition_matrix_t +
