...
void LaneDetector::calculate_pos(bool is_left) {
  if (is_left) {
    if (std::round(state_.left_slope_) == 0 &&
      ...
      state_.left_pos_ = static_cast<int32_t>(
          (offset - state_.left_intercept_) / state_.left_slope_);
    ...
    if (std::round(state_.right_slope_) == 0 &&
      ...
      state_.right_pos_ = static_cast<int32_t>(
          (offset - state_.right_intercept_) / state_.right_slope_);
